...
func (p *webhookProcessor) createDerivativeOrder(ctx context.Context, primaryOrder *models.Order, fillEvent *models.FillEvent, orderData *binance.WebhookOrder) error {
	derivativeQty := fillEvent.FillQuantity

	// Determine hedge direction (opposite to primary order)
	var derivativeDirection models.Direction
	switch primaryOrder.Direction {
	case models.DirectionLong:
		derivativeDirection = models.DirectionShort
	case models.DirectionShort:
		derivativeDirection = models.DirectionLong
	case models.DirectionCloseLong:
		derivativeDirection = models.DirectionCloseShort
	case models.DirectionCloseShort:
		derivativeDirection = models.DirectionCloseLong
	}

	// Determine derivative market and symbol
	var derivativeMarket models.MarketType
	var derivativeSymbol string
	var derivativeSymbolType string

	if primaryOrder.MarketType == models.MarketTypeSpot {
		// Spot -> Coin-M hedge
		derivativeMarket = models.MarketTypeCoinM

		contractType := p.cfg.Derivative.ContractType
		if contractType == "" {
			contractType = "current"
		}

		// Extract base currency (e.g., BTCUSDT -> BTC)
		base := extractBaseCurrency(primaryOrder.FullSymbol)

		// Query actual contract symbol
		quarterResp, err := p.binanceClient.CoinMGetQuarterSymbols(ctx, base)
		if err != nil {
			return fmt.Errorf("failed to get quarter symbols: %w", err)
		}

		if contractType == "current" {
			derivativeSymbol = quarterResp.Current.Symbol
		} else {
			derivativeSymbol = quarterResp.Next.Symbol
		}
		derivativeSymbolType = base + "-" + contractType

		// Convert quantity to contract units
		derivativeQty = p.convertToContractQty(derivativeQty, orderData.Price)

	} else {
		// Coin-M -> Spot hedge
		derivativeMarket = models.MarketTypeSpot
		base := extractBaseCurrency(primaryOrder.FullSymbol)
		derivativeSymbol = base + "USDT"
		derivativeSymbolType = derivativeSymbol
	}

	// Get current price
	var currentPrice string
	if derivativeMarket == models.MarketTypeSpot {
		priceResp, err := p.binanceClient.SpotGetPrice(ctx, derivativeSymbol)
		if err != nil {
			return fmt.Errorf("failed to get spot price: %w", err)
		}
		currentPrice = priceResp.Price
	} else {
		priceResp, err := p.binanceClient.CoinMGetPrice(ctx, derivativeSymbol)
		if err != nil {
			return fmt.Errorf("failed to get coinm price: %w", err)
		}
		currentPrice = priceResp.Price
	}
	...
	derivativeOrder := &models.Order{
		...
		ParentOrderID:         sql.NullInt64{Int64: int64(primaryOrder.ID), Valid: true},
		OrderRole:             models.OrderRoleDerivative,
		MarketType:            derivativeMarket,
		SymbolType:            derivativeSymbolType,
		FullSymbol:            derivativeSymbol,
		Direction:             derivativeDirection,
		OrderType:             models.OrderTypeLimit,
		Quantity:              derivativeQty,
		Price:                 sql.NullString{String: currentPrice, Valid: true},
		Status:                models.OrderStatusPending,
		FilledQuantity:        "0",
		TriggerFillEventID:    sql.NullInt64{Int64: int64(fillEvent.ID), Valid: true},
		PriceCalculationLogic: fmt.Sprintf("Market price at fill time: %s", currentPrice),
	}

	if err := p.orderRepo.Create(ctx, derivativeOrder); err != nil {
		return fmt.Errorf("failed to create derivative order: %w", err)
	}
	...
	p.createAuditLog(ctx, primaryOrder.FlowID, derivativeOrder.ID, models.LogTypeDerivativeTrigger, models.LogSeverityInfo,
		"Derivative order created", map[string]interface{}{
			...
			"direction":       string(derivativeDirection),
			"quantity":        derivativeQty,
			"price":           currentPrice,
		})

	// Mark fill event as having triggered a derivative
	fillEvent.HasTriggeredDerivative = true
	fillEvent.DerivativeOrderID = sql.NullInt64{Int64: int64(derivativeOrder.ID), Valid: true}
	p.fillRepo.Update(ctx, fillEvent)

	// Submit derivative order to Binance asynchronously
	go p.submitDerivativeOrder(context.Background(), derivativeOrder)
